...
- Use `uv run u2cli <command> [args]` to run commands.
- Use `uv run u2cli -s <serial> <command> [args]` when multiple Android devices are connected.
- Examples below show only `<command> [args]`; prepend `uv run u2cli` when running them.

## Commands

### Device Commands

- `dump-hierarchy`: Dump current UI hierarchy for analysis and element selection. The output is compressed by u2cli before returning. You can combine this command with `grep` command to find specific elements, like `dump-hierarchy | grep -i "keyword"`. If you want to get the full hierarchy, you can use `dump-hierarchy --raw`.
